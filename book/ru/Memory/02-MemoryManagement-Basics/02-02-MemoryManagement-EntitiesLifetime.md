...
Таким образом с точки зрения программиста, использующего объект возникает псевдодетерменированное разрушение объекта: перевод объекта в разрушенное состояние при помощи метода `Dispose()`. Память из-под объекта не освобождается, ссылка на объект всё еще выставлена: просто необходимо его разрушение в конкретной точке приложения. Например, потому что этот объект обязан отписаться от каких-то событий либо же освободить какой-то ресурс. 
